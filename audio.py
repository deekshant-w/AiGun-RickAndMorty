...
def main(callback: Callable):
    ...
    wake = WakeModel(wakeword_models=[WAKE_WORD], inference_framework="onnx") # It has it's own internal buffer storing frames till it detects the wake word
    stt = WhisperModel("small.en", device="cpu", compute_type="int8") # Audio to text
    vad = webrtcvad.Vad(2) # Checks if a frame contains speech. 0 is least aggressive about filtering out non-speech, 3 is most aggressive.

    pre_roll = collections.deque(maxlen=max(1, round(PRE_ROLL_S / (BLOCK / RATE)))) # store audio from before the wake word was detected, so we don't cut off the beginning of speech
